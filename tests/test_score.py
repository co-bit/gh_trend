...
def test_compute_star_momentum_ranks_between_absolute_and_relative_extremes():
    # 中規模の実質的な成長(865スターで+688)が、
    # 大規模の平常運転(198,894スターで+987)も
    # 極小規模のノイズ(5スターで+4、80%成長)も上回ること
    mid = scoring.compute_star_momentum(688, 865)
    assert mid > scoring.compute_star_momentum(987, 198894)
    assert mid > scoring.compute_star_momentum(4, 5)


def test_compute_star_momentum_handles_missing_and_zero_inputs():
