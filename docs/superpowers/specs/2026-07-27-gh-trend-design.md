...
GitHub Actions (daily cron, JST 9:00)
  ...
  │                     ・リポジトリごとにスレッドプールで並列実行(I/O待ちが
  │                       支配的なため。監視対象数が数千件規模になっても
  │                       実行時間が線形に伸びないようにする)
...
静的HTML1ファイルとして生成し、追加のサーバーサイド処理は不要(Google Fontsの読み込みのみ外部リソースに依存)。
...
- 監視対象リポジトリ数に上限を設けないため、長期的にリポジトリ数が増えると1日あたりのAPI呼び出し数・スクレイピング件数が増加する。discover.pyのページネーション追加により監視対象が数千件規模(約5,300件)まで急増したことを受け、collect.pyをスレッドプールで並列化して実行時間の線形増加を抑えている。将来さらに規模が拡大しボトルネックになった場合は上位N件への絞り込みを検討する
